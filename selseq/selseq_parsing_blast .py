...
class ParsingBlast():
    ...
    def parsing_generator(self):
        ...
        global id_list_ref    
        seq_store = pd.Series()
        #table_opened = open(REDATA_DIRECTORY + self.blast_tbl)
        with open(REDATA_DIRECTORY + self.blast_tbl) as table_opened:
            query_last = 'start'
    ...
    def seq_choice(self,seq_store_in):
        ...
        print(self.choiced,'===',self.id_query)
    ...
    def seq_choised_union(self,choiced):
        if len(self.choiced_full.values()) == 0:
            ...
            for val_set in (dict(self.choiced_full)).values():
                
                if not val_set.isdisjoint(choiced[self.id_query]):
                    print('val_set==',choiced)
                    self.choiced_full[self.id_query] = val_set.union(choiced[self.id_query])
...
pt = ParsingBlast('tbl.csv')
for i in pt.parsing_generator():
